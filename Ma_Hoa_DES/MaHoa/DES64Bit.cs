...
using Ma_Hoa_DES.HangSo;
namespace Ma_Hoa_DES.MaHoa
{
    internal class DES64bit
    {
        public Khoa KhoaDES { get; set; }
        public ChuoiNhiPhan ThucHienDES(Khoa key, ChuoiNhiPhan ChuoiVaoDai, int MaHoaHayGiaiMa)// 1 ma hoa, -1 giai ma
        {
            ...
            for (int k = 0; k < DSChuoiVao.Length; k++)  // duyêt qua từng chuỗi được chai
            {
                ...
                // b1: tính IP
                ChuoiSauIP = ThongSo.TinhIP(DSChuoiVao[k]);
                // lấy giá trị L,R
                L = ChuoiSauIP[0];
                R = ChuoiSauIP[1];
                ...
                // tính IP_1
                ChuoiSauIP_1 = ThongSo.TinhIP_1(R, L);
        ...
        public string ThucHienDESText(Khoa key, string ChuoiVao, int MaHoaHayGiaiMa)// 1 ma hoa, -1 giai ma
        {
            ...
            return ChuoiNhiPhan.ChuyenNhiPhanSangChu(KQ);// chuuyển sang dạng text để hiện thị kết quả
        }
        private ChuoiNhiPhan HamF(ChuoiNhiPhan chuoiVao, ChuoiNhiPhan KhoaCon)
        {
            ChuoiNhiPhan KQ = ThongSo.TinhE(chuoiVao); // Tính E trước
            KQ = KQ.XOR(KhoaCon); // tính XOR 
            KQ = ThongSo.TinhSBox(KQ); // tính hộp s-box
            KQ = ThongSo.TinhP(KQ); // tính P là ok
            return KQ;
        }
